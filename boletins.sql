...
--
-- Host: localhost:3306
-- Tempo de geração: 20/12/2019 às 00:45
...
--
DELIMITER $$
CREATE TRIGGER `tgr_delete_aluno` AFTER DELETE ON `alunos` FOR EACH ROW BEGIN
...
(2, 'Algoritmo e Estrutura de Dados I'),
...
--
DELIMITER $$
CREATE TRIGGER `tgr_delete_disciplina` AFTER DELETE ON `disciplinas` FOR EACH ROW BEGIN
